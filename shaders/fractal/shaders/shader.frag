...
float cube(vec3 p) {
  ...
  p.zx *= rot(pc.time * PI / 2);
  p.yx *= rot(pc.time * PI / 2);
  return max(-sc(p, 0.6), length(max(abs(p) - vec3(1.), 0.)));
...
void main() {
	...
	vec3 cuber = raymarch(uu);
	float edge = sin(dt + 2 * PI) * 2;
    vec3 col = (uu.x >= edge) ? cuber : PINK + square(uu);
    col = raymarch(uu);

	vec3 tex = T(prev_frame).brg;

	col = col + tex * 0.7;
    out_color = vec4(col * 1.2 / (2.1 - col * 0.5), 1.0);
    out_color = vec4(pow(col, vec3(3/0.1)), 1.0);
}
